...
import OeuvreNoSlide from "./OeuvreNoSlide";
...
export default function Artist(props) {
  ...
  return (
    <div className="border-t border-black pt-3 ml-3 mr-6 md:ml-6 md:mr-12 LibreBaskerville word-break">
      ...
      <div className='pb-3'>
      {!count(artiste) ? (
        <OeuvreNoSlide data={artiste} />
      ) : (
        <div className="navigation-wrapper mb-3 ">
          <div ref={sliderRef} className="keen-slider">
            {artiste.fields.titreOeuvre1 && (
              <Oeuvre numero={"1"} artiste={artiste} />
            )}

            {artiste.fields.titreOeuvre2 && (
              <Oeuvre numero={"2"} artiste={artiste} />
            )}

            {artiste.fields.titreOeuvre3 && (
              <Oeuvre numero={"3"} artiste={artiste} />
            )}

            {artiste.fields.titreOeuvre4 && (
              <Oeuvre numero={"4"} artiste={artiste} />
            )}

            {artiste.fields.titreOeuvre5 && (
              <Oeuvre numero={"5"} artiste={artiste} />
            )}

            {artiste.fields.titreOeuvre6 && (
              <Oeuvre numero={"6"} artiste={artiste} />
            )}

            {artiste.fields.titreOeuvre7 && (
              <Oeuvre numero={"7"} artiste={artiste} />
            )}
          </div>
